...
@st.cache_resource
def load_ollama_llm(model_name: str, base_url: str, temperature: float):
    return ChatOllama(
        model=model_name,
        base_url=base_url,
...
def rewrite_query_with_history(
    ...
    system_prompt = """
You rewrite follow-up questions for retrieval in a RAG system.

Rules:
- Do NOT answer the question.
- Return only one standalone retrieval query.
...
def generate_multi_queries(
    ...
    system_prompt = f"""
You generate alternative retrieval queries for a RAG system.

Rules:
- Do NOT answer the question.
- Output exactly {num_variants} alternative queries.
- One query per line.
- No numbering.
- No bullet points.
...
def build_generation_prompt(
    ...
    for i, doc in enumerate(retrieved_docs, start=1):
        ...
        shortened_content = doc.page_content[:800]
...
- Prefer 3 short bullet points when appropriate.
- Do not copy raw chunk headers or metadata into the main answer.
- At the end, add a short Sources section using source titles or source names only.
""".strip()
...
def fallback_answer_from_docs(
    ...
    lines = [
        "Local Ollama answer generation is currently unavailable.",
    ]
...
with st.sidebar:
    ...
    st.caption(
        "This app uses local Hugging Face embeddings, local Chroma retrieval, and local Ollama generation."
    )
...
if user_query:
    if not ollama_model_name.strip():
        st.warning("Please enter an Ollama model name in the sidebar.")
        st.stop()
